...
class MoonAppView_Moon_4 extends WatchUi.View {

    var Moon_4_Timer = new Timer.Timer();
   ...
   // Load your resources here
    function onLayout(dc as Dc) as Void {
        ViewTimeCounter = 0;
        setLayout(Rez.Layouts.Layout_Moon_4(dc));
    }
    ...
    function onShow() as Void {
        ViewTimeCounter = 0;
        Moon_4_Timer.start(method(:Moon_4_Timer_Callback), 1000, true); // a 1-minute timer
    }
    ...
    function onUpdate(dc as Dc) as Void {
        ...
        dc.setPenWidth(1);
        for (var i = 0; i <= 8; i++) {
            dc.drawCircle(140 * sc, 140 * sc, 140 * sc - 15 * sc * i);
        }
        ...
        // draw azimuth line
        var Azi = Moon["MoonDay"];
        var AziPoints = getMoonAzPoints(Azi);
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        dc.setPenWidth(2 * sc);
        for (var i = 0; i < AziPoints.size() - 1; i++) {
            if ( !(Azi[i+1][1] < 90 && Azi[i][1] > 270) && !(Azi[i+1][1] > 270 && Azi[i][1] < 90)) {
                dc.drawLine(AziPoints[i][0] * sc, AziPoints[i][1] * sc, AziPoints[i+1][0] * sc, AziPoints[i+1][1] * sc);
            }
        }
        ...
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_BLACK);
        for (var i = 1; i <= 8; i++) {
            dc.drawAngledText(140 * sc, 280 * sc - 15 * sc * i + 5 * sc, DawnPhasesTimeFont, Direction(360-45*i), Graphics.TEXT_JUSTIFY_CENTER, 0);
        }
        ...
        if (Moon["Moon_Today_rise_bool"] == true) {
            ...
            H24angle += (H24angle < 0) ? 360 : 0;
            H24direction = (H24angle <= 180) ? Graphics.RADIAL_TEXT_DIRECTION_CLOCKWISE : Graphics.RADIAL_TEXT_DIRECTION_COUNTER_CLOCKWISE;
            H24radius = 45 + 12 * H24direction;
            dc.drawRadialText(140 * sc, 140 * sc, MoonTimeFont2, "R", Graphics.TEXT_JUSTIFY_CENTER, H24angle, H24radius * sc, H24direction);
        }
        if (Moon["Moon_Today_set_bool"] == true) {
            ...
            H24direction = (H24angle <= 180) ? Graphics.RADIAL_TEXT_DIRECTION_CLOCKWISE : Graphics.RADIAL_TEXT_DIRECTION_COUNTER_CLOCKWISE;
            H24radius = 45 + 12 * H24direction;
            dc.drawRadialText(140 * sc, 140 * sc, MoonTimeFont2, "S", Graphics.TEXT_JUSTIFY_CENTER, H24angle, H24radius * sc, H24direction);
        }
        if (Moon["Moon_Today_transit_bool"] == true) {
            ...
            H24direction = (H24angle <= 180) ? Graphics.RADIAL_TEXT_DIRECTION_CLOCKWISE : Graphics.RADIAL_TEXT_DIRECTION_COUNTER_CLOCKWISE;
            H24radius = 45 + 12 * H24direction;
            dc.drawRadialText(140 * sc, 140 * sc, MoonTimeFont2, "T", Graphics.TEXT_JUSTIFY_CENTER, H24angle, H24radius * sc, H24direction);
        }
    ...
    function Moon_4_Timer_Callback() {
        ViewTimeCounter++;
